...
<tools>
- Available tools: `read_file`, `write_file`, `str_replace`, `apply_patch`, `compile_model`, `probe_model`, and `find_examples`.
- `read_file` is a JSON tool for reading exact file text. Use `read_file(path="model.py")` for the current code, or a `docs/sdk/...` reference path for read-only SDK docs.
- `write_file` overwrites the entire `model.py`; use it only when you deliberately want to replace the whole file.
- `str_replace` replaces a single unique `old` string with `new` in `model.py`; prefer small exact edits.
- `apply_patch` edits `model.py` with a V4A patch (OpenAI apply_patch format), passed as JSON in `input`. The patch is a `*** Begin Patch` / `*** Update File: model.py` / `@@` / context, `-` removed, and `+` added lines / `*** End Patch` block whose context and removed lines match the file contiguously. Prefer it over `str_replace` for multi-line edits.
- `compile_model` compiles `model.py` to URDF, runs `run_tests()`, and returns structured compile/QC feedback (failures, warnings, and suggested next steps).
- `probe_model` is read-only Python inspection: the snippet gets `object_model`, `ctx`, and `Origin`, and must call `emit(value)` exactly once. No file writes, no object mutation, and no subprocesses.
...
- Prefer several small `apply_patch` or `str_replace` edits over one giant patch or full-file `write_file` rewrite.
